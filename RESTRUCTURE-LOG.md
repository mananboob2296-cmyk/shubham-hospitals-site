...
- [x] **S5 — SEO/GEO/AEO/AIO audit + security check** (done 2026-07-17). Findings below.
  - **Good / no action:** per-page canonical, hreflang trio + x-default, OG/twitter, and JSON-LD all present and correct on live pages (verified en AMH post + hi high-risk post). MedicalClinic schema sitewide; BlogPosting+FAQPage on posts; Physician on doctors. `sitemap-index.xml` live, `robots.txt` points to it. Emails obfuscated by Cloudflare (cdn-cgi/l/email-protection) — fine.
  - **Live headers + llms.txt RE-VERIFIED via Chrome (no-store fetch, 2026-07-17):** all 5 security headers present on live home response (HSTS, X-Frame-Options SAMEORIGIN, X-Content-Type-Options nosniff, Referrer-Policy, Permissions-Policy); TLS via Cloudflare; no CSP (optional to add). `/llms.txt` served live returns the CORRECT Shubham content — an earlier "stale Hostinger staging" result was only a WebFetch-cache artifact, NOT a real problem. No cache purge needed for llms.txt.
  - **BUG FOUND + FIXED (commit `62163fe`, 2026-07-17):** the S4 disclaimer-fix upload (commit `86d4ef9`) overwrote the 4 **hi** posts (high-risk-pregnancy, laparoscopy-vs-open, AMH-level, fertility-specialist-7-signs) with the **English** bodies + `lang: en` — so those 4 hi URLs were serving English text with wrong lang/hreflang. Restored Hindi bodies from git history (`362e53c` parent), set `lang: hi`, added the Hindi in-body disclaimer to each. en (4) and mr (5) files from the same S4 batch were correct. (Cloudflare will redeploy from main; owner may Purge Everything to speed it up.)
  - **AEO/AIO — real finding, owner decision:** live `robots.txt` has Cloudflare "Managed Content" AI-bot blocks (ClaudeBot, GPTBot, Google-Extended, CCBot, Bytespider, Applebot-Extended, meta-externalagent, etc. → Disallow) prepended ahead of the repo's allow-all, plus `Content-Signal: ai-train=no`. This is the Cloudflare AI-crawler toggle, not the repo file. **For AEO/AIO you generally WANT AI crawlers allowed. If you want ChatGPT/Claude/Perplexity/Google AI Overviews to read & cite the site, turn OFF Cloudflare's "block AI bots" setting (dash → domain → Security/Bots, or the robots.txt manager).** Trade-off: blocking protects content from AI training but also blocks AI-answer citations.
  - **Minor / optional:** CLINIC_SCHEMA `sameAs` could add YouTube + Google profile; consider `aggregateRating` once real review data available; optional CSP header.
...
- 2026-07-17 i: S5 DONE — SEO/AEO/security audit. Head OK, schema OK; live security headers + llms.txt re-verified via Chrome (all good). Found + fixed 4 hi posts that S4 upload had overwritten with English (commit `62163fe`). One owner decision flagged: Cloudflare currently blocks AI crawlers (ClaudeBot/GPTBot etc.) — turn OFF for AEO/AIO citations. Next: S6.
